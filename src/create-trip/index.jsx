...
function CreateTrip() {
  ...
  const OnGenerateTrip = async () => {
    ...
    const rawText  = await result.response.text();   // <- await here
SaveAiTrip(rawText);
  }
...
const SaveAiTrip = async (TripData) => {
  ...
  console.log("SaveAiTrip called with:", TripData?.slice?.(0, 80)); // <= should print
  setLoading(true);
  ...
  try {
    ...
    await setDoc(
      ...
      { merge: false }           // optional; just explicit
    );

    navigate("/view-trip/" + docId);
  } catch (err) {
    console.error("Firestore write failed:", err.code, err.message); // <-- see the code!
    toast.error("Could not save trip: " + err.code);
  } finally {
    setLoading(false);
  }
};
